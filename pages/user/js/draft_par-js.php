<script>
	...
	$(document).ready(function() {
		...
		var yer = $('#year option:selected').text();
		if (yer.length > 4) {
			var result = yer.slice(-5)
			$('#year option:selected').text(result);
		}
	...
	$('#btnSubmit').on('click', function(e) {
		...
		if (name != '' && position != '' && department != '' && project != '' && status != '-Select Employment Status-' && assessment != null && from != '' && to != '' && date_hire != '' && rater != '' && emp_email != '') {
			//KRA & KPI section
				...
				if (kra1 != '' || kpi1 != '' || kra_rating1 != '' || kra2 != '' || kpi2 != '' || kra_rating2 != '' || kra3 != '' || kpi3 != '' || kra_rating3 != '') {
					if (gp1a_rate != '' || gp1b_rate != '' || gp1c_rate != '' || gp2a_rate != '' || gp2b_rate != '' || gp2c_rate != '' || gp3a_rate != '' || gp3b_rate != '' || gp3c_rate != '' || gp4a_rate != '' || gp4b_rate != '' || gp4c_rate != '' || gp5a_rate != '' || gp5b_rate != '' || gp5c_rate != '' || gp6a_rate != '' || gp6b_rate != '' || gp6c_rate != '' || gp7a_rate != '' || gp7b_rate != '' || gp7c_rate != '' || gp8a_rate != '' || gp8b_rate != '' || gp8c_rate != '' || gp9a_rate != '' || gp9b_rate != '' || gp9c_rate != '' || gp10a_rate != '' || gp10b_rate != '' || gp10c_rate != '') {
						if (emp_comment.length > 10) {
							...
							const comments1_msg = $('#comments1_msg').is(':visible');
							const comments2_msg = $('#comments2_msg').is(':visible');
							const comments3_msg = $('#comments3_msg').is(':visible');
							const comments4_msg = $('#comments4_msg').is(':visible');
							const comments5_msg = $('#comments5_msg').is(':visible');
							const comments6_msg = $('#comments6_msg').is(':visible');
							var action_s = 3; //Submit
	...
	$('#btnDraft').on('click', function(e) {
		...
		if (name != '' && position != '' && department != '' && project != '' && status != null && assessment != null && from != '' && to != '' && date_hire != '' && rater != '' && rater_name != null && emp_email != '') {
			...
			if (emp_comment.length != 0) {
				...
				var action_s = 4; //Draft
				// RESTRICTIONS For EMPTY RATING COMMENT WITH THE SUBMIT AJAX
				scrollUpEmptyComments_WithAjax(comments1_msg, comments2_msg, comments3_msg, comments4_msg, comments5_msg, comments6_msg, myData, action_s);
			} else {
				$('#emp_message2').attr("hidden", false);
				// toastr.error("Your employee's comment must have at least a minimum of 10 characters.")
			}
		} else {
			toastr.error('ERROR! Please fill out all the data needed in employee details section.');
